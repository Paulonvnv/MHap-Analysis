...
cigar_object = read_cigar_tables(files = 'docs/data/Pvivax/dada2/run_dada2/CIGARVariants_Bfilter.out.tsv', sample_id_pattern = 'SP|CON|HR')
```
...
```{r}
evectors = fastGRM(gt = loci_object$loci_table, k = nrow(loci_object$loci_table),
                   metadata = ampseq_filtered@metadata,
                   Pop = 'Population', q = 2)

evectors %>% ggplot(aes(x = PC1, y = PC2, color = Population))+
...
evectors_IBD = fastIBD_PCA(ampseq_object = ampseq_filtered, relatedness_table = pairwise_relatedness, k = nrow(ampseq_filtered@metadata), Pop = 'Population', q = 2)
