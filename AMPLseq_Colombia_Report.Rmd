...
external_metadata %<>% mutate(
  ...
  grepl('SP0112286',samples) ~ 'Quibdo',
  grepl('SP0126288',samples) ~ 'Guapi'),
  date_of_symp = case_when(
    X2.9..Fecha.inicio.de.síntomas != "" ~ X2.9..Fecha.inicio.de.síntomas,
    X2.9..Fecha.inicio.de.síntomas == "" ~ X2.3..Fecha.de.reclutamiento.
 ))

external_metadata %<>% select(samples, Population, date_of_symp)
external_metadata %<>% mutate(month_of_symp = substr(date_of_symp, 1, 7))
external_metadata[["group"]] = ifelse(external_metadata$samples %in% cigar_object$metadata$samples, "AMPseq", "noAMPseq")

# Merging metadata
cigar_object$metadata = merge(cigar_object$metadata, external_metadata[,c('samples', 'date_of_symp', 'month_of_symp')], by = 'samples', all.x = TRUE)

# Imputing missing data with the previous observation
cigar_object$metadata[which(is.na(cigar_object$metadata$month_of_symp) & cigar_object$metadata$typeofSamp == 'Samples'),][['month_of_symp']] = cigar_object$metadata[which(is.na(cigar_object$metadata$month_of_symp) & cigar_object$metadata$typeofSamp == 'Samples')-1,][['month_of_symp']]
...
plot_temporal_collection_of_samples = external_metadata %>%
  ...
  group_by(Population, month_of_symp) %>%
  summarise(nsamples= n())%>%
  ggplot(aes(x = month_of_symp, y = nsamples, fill = factor(Population,
                                                              levels = c("Quibdo",
                                                                         "Buenaventura",
                                                                         "Guapi"))))+
  geom_col()+
...
For that purpose we are going to use the function `fx_cigar2loci_abd.R`. This function requires 4 arguments. First a `cigar_object` (containing the cigar_table and the metadata). Then `markers`, which is a table with the set of markers and their names (in a column named `amplicon`). This argument is optional, and in case is `NULL`, the name of the amplicons will be extracted from the columns in the `cigar_object`. The next arguments are `min_abd` and `min_ratio`, which are the minimum abundance (minimum read count) required to call an allele and the minimum ratio between the minor and major alleles in a polyclonal sample. By default these two arguments are 10 and 0.1 respectively. This function automatically differentiate between controls and samples of interest, and its output is a list with 6 slots: `loci_abd_table` where the genotypic information of the samples of interest is stored in a loci_abd format; the `metadata` of the samples of interest; `control` with all the information of the controls; `markers` containing the information of the markers such as the chromosome location; and two empty slots required for the next steps which are `loci_performance` and `pop_summary`.
...
Once the data has been converted to the loci_abd format, we can use the function `fx_loci_amplification_rate` to measure the proportion of samples that have been amplified by each 
...
drug_haplotypes_respect_to_3D7 = fx_haplotypes_respect_to_reference(ampseq)
