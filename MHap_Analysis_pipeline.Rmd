...
1.  **Upload**: Data can be uploaded in up to 4 different ways using the
    ...
    7.  Two additional parameters are needed when uploading data:
        ...
        2.  **markers**: Name of the csv. table that contains
            ...
            of markers extracted from the cigar table.
...

{
  ...
  //Upload of raw data
  "cigar_paths": null, // Path to output folders of the denoising pipeline, e.i.: "Seq_runs", null
  
  "cigar_files": "cigar_tables",//"cigar_tables", null
  "asv_table_files": "asv_tables",//"asv_tables", null
  "asv2cigar_files": "asv2cigar",//"asv2cigar", null
  "asv_seq_files": "asv_seqs",//"asv_seqs", null
  "zero_read_sample_list": "zeroReadSamples",//"zeroReadSamples", null
...
the pipeline is controlled by the this bash script which can be launched
from the terminal using the following line:

```{bash, eval = FALSE}
./run_mhap.sh MHap_Analysis_inputs.json
```
