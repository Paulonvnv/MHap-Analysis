...
drug_resistant_haplotypes = fx_drug_resistant_haplotypes(ampseq,
                                        ...
                                        variables = c('samples', 'Population', 'quarter_of_collection'), na.var.rm = TRUE,
                                        filters = c('Population;Buenaventura,Quibdo,Guapi',                               'quarter_of_collection;2020-Q4,2021-Q1,2021-Q2,2021-Q3,2021-Q4'))
...
plot_log_NPolyLoci_by_pop = log_scale_histogram(data = ampseq$metadata[ampseq$metadata$NPolyLoci != 0,],
                                                ...
                                                levels = c("Quibdo", "Buenaventura", "Guapi", "missing data"), x_label = "Number of heterozygous loci per sample",
...
plot_log_coi_by_pop = log_scale_histogram(data = ampseq$metadata,
                                          ...
                                          binwidth = 1,
                                          group_by = "Population",
                                          levels = c("Quibdo", "Buenaventura", "Guapi", "missing data"), x_label = "COI",
...
plot_poly_by_pop = ampseq[["pop_summary"]] %>% ggplot(aes(x = factor(pop, levels = c("Quibdo", "Buenaventura", "Guapi", "Total")),
                                                          y = prop_poly,
                                                          fill = factor(pop, levels = c("Quibdo", "Buenaventura", "Guapi", "missing data","Total"))))+
  geom_col(alpha = .6) +
  ...
  scale_fill_manual(values = c("firebrick3", "dodgerblue3", "gold3", "gray75","gray30"))+
...
plot_poly_by_pop_over_time = fx_get_polygenomic(ampseq_object = ampseq, strata = "Pop_quarter", update_popsummary = F, na.rm = TRUE)[-1,]%>%mutate(
