...
filter_loci = function(obj, v, update_cigars = TRUE, skip_errors = FALSE){
  ...
  if(class(obj) == 'ampseq'){
    ...
    if(update_cigars){
      ...
      cigars_gt = get_cigar_alleles(ampseq_object = obj2, 
                                    from = 'gt',
                                    as = 'vector')
                      ...
                      for(i in 1:length(temp_replaced_allele)){
                        ...
                        for(sample in 1:nrow(gt_masked)){
                          ...
                          if(grepl(paste0('^',replaced_pattern, ':'), gt_masked[sample,mhap])){
                            ...
                            gt_masked[sample,mhap] = gsub(paste0('^', sample_replaced_pattern), sample_replacement_pattern, gt_masked[sample,mhap])
...
haplotypes_respect_to_reference = function(ampseq_object,
  ...
  if(plot_haplo_freq){
    ...
    if(length(gene_ids) > 1){
      ...
      #                            dimnames = list(
      #                              rownames(aacigar_table)[(apply(aacigar_table, 1, function(i){sum(grepl("\\|",i))}) == 0)],
      #                              gene_ids
      #                            ))
      poly_aacigar_table = aacigar_table[(apply(aacigar_table, 1, function(i){sum(grepl("\\|",i))}) != 0), , drop = FALSE]
      ...
      #                             dimnames = list(
      #                               rownames(dnacigar_table)[(apply(dnacigar_table, 1, function(i){sum(grepl("\\|",i))}) == 0)],
      #                               gene_ids
      #                             ))
      poly_dnacigar_table = dnacigar_table[(apply(dnacigar_table, 1, function(i){sum(grepl("\\|",i))}) != 0), , drop = FALSE]
...
rGenome2loci = function(rGenome_object, allele_counts){
  ...
  markers %<>% S4Vectors::rename('CHROM' = 'chromosome',
                                 'POS' = 'pos')
...
get_ampseq_limit_of_detection = function(ampseq_object,
  ...
  if(for_procedure){
    ...
    # sample_performance %>% 
    #   filter(Strata == 'Colombia') %>%
    #   ggplot(aes(x = log_Concentration, y = log_OR_AmplRate)) + geom_point()
      ...
      for(Pop in unique(sample_performance$Strata)){
        for(RD_Thres in read_depth_thresholds){
          ...
          for(AmpRate_Threshold in AmpRate_Thresholds){
            ...
            #seq(1, max(sample_performance$Concentration), 0.1)
